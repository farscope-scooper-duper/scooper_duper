...
while ((time.time() - run_time) < time_limit) and (len(world_model.pick_list) > 0): #For 10 minutes, or until everything's picked
    if (state == 'get_target_item'):
    ...
    elif (state == 'move_to_bin'): #TODO: Needs to send the actual move instruction (coords of bin from lookup table)
        bin_reached = True #TODO: update this with correct bin-reached signal
        ...
        item_reached = True #TODO: update this with correct item reached signal
